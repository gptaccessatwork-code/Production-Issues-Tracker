...
class ToggleDropdown(ctk.CTkFrame):
    ...
    def _show(self):
        ...
        self._popup.geometry(f"{bw}x{len(self._values)*32}+{x}+{y}")
        self._popup.configure(bg=C["border"])
        self._popup.lift()
        inner = tk.Frame(self._popup, bg=C["surface"])
        inner.pack(fill="both", expand=True, padx=1, pady=1)
        for val in self._values:
            rf = tk.Frame(inner, bg=C["surface"], cursor="hand2")
            rf.pack(fill="x")
            lb = tk.Label(rf, text=f"  {val}", bg=C["surface"], fg=C["text"],
                          font=(F["family"], F["size_sm"]), anchor="w", height=2)
            lb.pack(fill="x")
            for w2 in (rf, lb):
...
class App(ctk.CTk):
    ...
    def _build_toolbar(self):
        ...
        _lbl(bar, "Hold Ctrl / Shift to select multiple rows",
             size=F["size_sm"], color=C["subtle"]).pack(side="right")
